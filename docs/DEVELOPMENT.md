...
- **无鉴权 / 多租户**：当前是单实例只读展示 + AI；生产多租户、团队协作、审批流仍是路线图。
...
- **知识图谱接真实数据**：`src/lib/graph.ts` 从库内企业↔行业↔城市↔竞争↔出口市场↔供应商关系生成图；`graph-view.tsx` 用 Fruchterman–Reingold 力导向布局 + 点击高亮邻居。
...
src/
  app/
    layout.tsx                 根布局（Providers + Shell）
    page.tsx                   首页（server 取数）
    home-view.tsx              首页视图（client：指标/汇率/新闻/热力图）
    api/chat/route.ts          DeepSeek 流式聊天路由（含 RAG 检索注入 + 来源推送）
  ...
  components/
    shell.tsx  ai-panel.tsx  ui.tsx  china-map.tsx
  db/
    schema.ts                  13 张表（含 rag_docs 向量表）+ $inferSelect 行类型
...
drizzle/                       生成的 SQL 迁移（0000–0005）
docs/                          本文档 + DATA_SOURCES.md
docker-compose.yml  drizzle.config.ts  .env.local(.example)
```
