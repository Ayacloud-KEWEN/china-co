...
从 Wikidata 按 **P442（中国行政区划代码）** 精确匹配，填 **人口(P1082) / 面积(P2046) / 英文名**。
...
- **GDP 不在此列**：全 Wikidata 带区划代码又有 GDP(P2131) 的条目不足 40 个，无法填充，
  只能人工在后台录入。省级 GDP 另有 `provinces` 表（由 `db:ingest` 填充）。
