...
其余（起库 → migrate → 种子 → 行政区划 → 摄取 → 构建 → 建向量索引 → PM2 启动 → 冒烟测试）
...
> **每次部署都会跑 `npm run db:divisions`**（导入全国行政区划，约 3.4k 行）。
> 它是 upsert：只刷新名称与上下级关系，**后台填写的区域情报不会被覆盖**。
> 拉取失败不中断部署（沿用库里已有数据）。详见第 8.5 节。
...
`db:setup` **不含**行政区划导入，首次部署后补跑一次（`deploy.sh` 会自动执行）：
```bash
npm run db:divisions
docker exec china-mos-db psql -U chinamos -d chinamos -c "select level, count(*) from divisions group by level;"
# province 31 / city 342 / district 3056
```
...
## 8.5 全国行政区划（`divisions` 表）

`/cities` 底部的省 → 市 → 区县树，以及 `/admin/divisions` 的录入界面，数据都来自 `divisions` 表。

```bash
npm run db:divisions          # 结构：省/市/区县的名称与上下级关系
npm run db:divisions:enrich   # 属性：人口/面积/英文名（Wikidata，约 3–5 分钟）
```

### 结构导入 `db:divisions`

- **数据源**：国标区划代码（GB/T 2260）公开 JSON 数据集，需服务器能出站访问 GitHub raw。
- **规模**：31 省 + 342 地级市 + 3056 区县 = **3429 行**（不含港澳台，如需可在后台手工补）。
- **幂等**：upsert 只写 `name` / `parent_code` / `level`。后台填写的
  `gdp` / `pop` / `pillars` / `summary` / `notes` / `city_slug` **永不被覆盖**，可放心反复运行。
- **层级可扩展**：表是自引用结构（`code` / `parent_code` / `level`），
  以后要加乡镇街道只需换数据源重跑，**无需改 schema、无需新迁移**。
- 若服务器无法访问 GitHub raw：在本地跑一次导出，再把 `divisions` 表用
  `pg_dump -t divisions` / `psql` 灌到服务器（第 13 节备份命令同理）。

### 属性填充 `db:divisions:enrich`

从 Wikidata 按 **P442（中国行政区划代码）** 精确匹配抓取结构化字段，再按维基条目链接
从 **Wikipedia** 抓三语摘要。实测覆盖率：

| 字段 | 省 | 地级市 | 区县 |
|---|---|---|---|
| 人口 P1082 / 面积 P2046 | 100% | 97% | 94% |
| 三语概述（Wikipedia 首段） | 100% | 97% | 92–94% |
| 经纬度 P625 | 100% | 97% | 94% |
| 政府官网 P856 | 100% | 96% | 91% |
| 邮政编码 P281 | 13% | 95% | 91% |
| 电话区号 P473 | 13% | 87% | 37% |
...
- **默认只填空白**，`/admin/divisions` 手动改过的值不会被覆盖；加 `-- --force` 才强制刷新。
- 全部填满后再次运行会在几秒内跳过，所以放进 `deploy.sh` 每次跑也不拖慢部署。
- Wikidata 没有对应条目的区县（约 6%）**保持空白**，不猜测填充。
- **概述为百科口径**（历史沿革/地理），非招商口径。自动填入的会标记 `summary_source='wikipedia'`，
  前台标注「摘要来源：维基百科」；**在后台改写后标记即清除**，之后重跑导入不再覆盖。
- **市/县 GDP 无免费源**：实测省级 90% 有 P2131，地级市与区县均为 **0%**，
  只能人工在后台录入。省级 GDP 另有独立的 `provinces` 表（由 `db:ingest` 填充）。
- **支柱产业**同样无结构化源，需人工填写。
- 首次运行约 **8–12 分钟**（Wikidata 分批 SPARQL + 三语 Wikipedia 摘要），请勿中断。

### 后台录入

`/admin/divisions` → 逐级下钻或搜索 → 编辑。可填 GDP、支柱产业、三语概述、备注。
填了「关联城市 Slug」的行政区，前台点击会直接跳到对应的完整城市情报页。
...
| `db:migrate` 报 `42P07 already exists` / `42P01 does not exist` | **迁移记录断层**：该库当初是用 `db:push` 建的，`drizzle.__drizzle_migrations` 是空的，migrate 会从 0000 重放。见下方「迁移记录断层」 |
| `divisions` 表不存在 / `/cities` 无区划树 | 迁移未应用：见「迁移记录断层」手动执行 `drizzle/0013_*.sql`，再 `npm run db:divisions` |
...
### 迁移记录断层（`db:migrate` 报表已存在 / 表不存在）

`drizzle-kit migrate` 靠 `drizzle.__drizzle_migrations` 记录已应用的迁移。
**如果这个库当初是用 `db:push` 建起来的**（而非 `migrate`），该表是空的，
于是 migrate 会尝试从 `0000` 重放全部迁移 → 撞上已存在的表而报错，新迁移也就没被应用。

先确认属于哪种情况：
```bash
docker exec china-mos-db psql -U chinamos -d chinamos -c "select count(*) from drizzle.__drizzle_migrations;"
```
- **返回 ≥ 13**：记录正常，`./deploy.sh` 会自动应用新迁移，无需处理。
- **返回 0 或表不存在**：存在断层，手动只应用缺失的那个迁移（0013 是纯新增，无破坏性）：

```bash
docker exec -i china-mos-db psql -U chinamos -d chinamos -v ON_ERROR_STOP=1 < drizzle/0013_burly_rictor.sql
npm run db:divisions
```

> 长期建议：找个维护窗口把迁移记录补齐（按实际库结构手工写入
> `drizzle.__drizzle_migrations`），否则以后每次新增迁移都要手动应用。
...
- [ ] `divisions` 表已导入（3429 行，`/cities` 底部出现区划树）
- [ ] `npm run build` 通过
