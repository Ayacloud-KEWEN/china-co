...
type Row = Pick<Division, "code" | "parentCode" | "level" | "name" | "citySlug" | "gdp" | "pop" | "area" | "website" | "notes"> & {
  pillars: string[];
  hasSummary: boolean;
};
...
const isFilled = (r: Row) => !!(r.gdp || r.pop || r.area || r.website || r.pillars.length || r.notes || r.hasSummary || r.citySlug);
              ...
              return (
                <tr key={r.code} className="border-t hover:bg-background/50">
                  ...
                  <td className="hidden px-4 py-2 text-xs text-muted sm:table-cell">
                    {isFilled(r)
                      ? [r.gdp && "GDP", r.pop && "人口", r.area && "面积", r.website && "官网", r.pillars.length > 0 && "产业", r.hasSummary && "概述", r.notes && "备注", r.citySlug && `→ ${r.citySlug}`].filter(Boolean).join(" · ")
                      : "—"}
                  </td>
                  <td className="px-4 py-2">
