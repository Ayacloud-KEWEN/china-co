...
| 模块 | 路由 |
| --- | --- |
| 首页仪表盘（Hero + AI 搜索 + 指标趋势 + 汇率 + 新闻 + 真实地图） | `/` |
| AI 全局搜索（向量 RAG + 工具调用 + 引用） | `/search` |
| 企业情报（AI 档案/SWOT/五力 + 财务/股价/股权/专利） | `/companies` |
| 行业情报（研究趋势 + 逐年贸易） | `/industries` |
| 城市情报（省级 GDP 榜 + OSM 工业区地图 + 全国省/市/区县行政区划树） | `/cities` `/cities/area/[code]` |
| 供应链 Intelligence（供应商目录 + AI 推荐/RFQ） | `/supply-chain` |
| 政策法规 Intelligence | `/policy` |
| 商机中心（展会 + 招投标） | `/opportunities` |
| Playbooks 攻略中心（AI 生成攻略） | `/playbooks` |
| 知识图谱（真实关系 + 力导向 + 股权） | `/graph` |
| 对比分析（企业/行业并排 + AI 点评） | `/compare` |
| AI 咨询顾问 | `/consultant` |
| AI 报告生成（真实 Word/PDF 导出） | `/reports` |
| 咨询工作台 | `/workspace` |
| 我的空间（关注/笔记/保存分析/团队） | `/me` |
| 登录 / 注册 | `/login` `/signup` |
...
# 一键：起库 + 建表(migrate) + 种子 + 摄取真实数据 + 建向量索引（需 Docker Desktop 运行中）
npm run db:setup
...
| `npm run db:divisions` | 导入全国行政区划（省/市/区县 3429 行，幂等，不覆盖后台填写内容） |
| `npm run db:studio` | 打开 Drizzle Studio 可视化查看数据 |
| `npm run db:setup` | 起库 + 建表 + 种子 + 真实数据摄取，一键完成 |

- Schema：`src/db/schema.ts`（三语字段用 JSONB 存储）
- 行政区划：`divisions` 表为**自引用**结构（`code` / `parent_code` / `level`），
  当前灌到区县级；扩展到乡镇街道只需换数据源重跑 `db:divisions`，无需改 schema。
  各级情报（GDP/人口/支柱产业/概述/备注）在 `/admin/divisions` 后台逐步填写。
...
- **账户与多租户**：注册/登录（自建 scrypt 哈希 + DB 会话 + httpOnly cookie，无需 OAuth key）；每个账户归属一个团队（organization）。
- **关注 / 笔记 / 保存的分析**：在企业/行业/城市详情页「关注」并记私人笔记；AI 回答可「保存分析」并选择**共享给团队**；`/me` 我的空间汇总关注列表、笔记、保存的分析（含团队共享）与成员。
